...
const Revisions = ({ onBack, onQuiz, onCours }) => {
  ...
  // Gestion des paramètres URL pour le chargement automatique de modules
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search)
    const moduleParam = urlParams.get('module')
    
    if (moduleParam) {
      // Exemple: mathematiques-6ieme-nombres-entiers
      if (moduleParam.startsWith('mathematiques-6ieme')) {
        setSelectedMatiere('Mathématiques')
        setSelectedNiveau('6ème')
      }
    }
  }, [])
