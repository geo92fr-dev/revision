...
import { TEST_CONFIG, TestUtils } from './config.js';
...
async function runFinalTests() {
    try {
        ...
        logTest('Nombre de topics', topics.length === TEST_CONFIG.EXPECTED_COUNT, `${topics.length}/${TEST_CONFIG.EXPECTED_COUNT}`);
        
        // Test 3: Vérification exports individuels
        console.log('\n🔍 Test 3: Vérification des exports...');
        let exportsPassed = 0;
        
        for (const topic of TEST_CONFIG.EXPECTED_TOPICS) {
            try {
                const module = await import(`${TEST_CONFIG.DATA_PATH}/${topic}.js`);
        ...
        logTest('Exports cohérents', exportsPassed === TEST_CONFIG.EXPECTED_COUNT, `${exportsPassed}/${TEST_CONFIG.EXPECTED_COUNT}`);
